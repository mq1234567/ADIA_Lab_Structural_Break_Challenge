...
def emit_train_datasets(X_train: pd.DataFrame, y_train_index: pd.DataFrame, selected_ids: Optional[List[str]] = None) -> List[Tuple[str, np.ndarray, np.ndarray, Optional[int]]]:
...
def emit_infer_datasets(X: pd.DataFrame, selected_ids: Optional[List[str]] = None) -> List[Tuple[str, np.ndarray, np.ndarray]]:
...
def local_ts_auc(y_test: pd.DataFrame, prediction: pd.DataFrame) -> float:
    ...
    '''
    y_test: pd.DataFrame with index (id, time) and columns ['target']
    prediction: pd.DataFrame with index (id, time) and columns ['prediction']
    '''
